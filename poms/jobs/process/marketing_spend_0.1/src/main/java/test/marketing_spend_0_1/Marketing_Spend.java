...
package test.marketing_spend_0_1;
...
/**
 * Job: Marketing_Spend Purpose: <br>
 * Description: <br>
 * 
 ...
 * @status
 */
public class Marketing_Spend implements TalendJob {
	...
	private final String jobName = "Marketing_Spend";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					Marketing_Spend.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(Marketing_Spend.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class row1Struct implements routines.system.IPersistableRow<row1Struct> {
		final static byte[] commonByteArrayLock_TEST_Marketing_Spend = new byte[0];
		static byte[] commonByteArray_TEST_Marketing_Spend = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_TEST_Marketing_Spend.length) {
					if (length < 1024 && commonByteArray_TEST_Marketing_Spend.length == 0) {
						commonByteArray_TEST_Marketing_Spend = new byte[1024];
					} else {
						commonByteArray_TEST_Marketing_Spend = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_TEST_Marketing_Spend, 0, length);
				strReturn = new String(commonByteArray_TEST_Marketing_Spend, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_TEST_Marketing_Spend, 0, length);
				strReturn = new String(commonByteArray_TEST_Marketing_Spend, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_TEST_Marketing_Spend) {
	...
	public void tFileInputExcel_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				String dbname_tDBOutput_1 = "Event_SA";
				String url_tDBOutput_1 = "jdbc:sqlserver://" + "localhost";
				if (!"".equals(port_tDBOutput_1)) {
					url_tDBOutput_1 += ":" + "1433";
				}
				if (!"".equals(dbname_tDBOutput_1)) {
					url_tDBOutput_1 += ";databaseName=" + "Event_SA";
				...
				final String decryptedPassword_tDBOutput_1 = routines.system.PasswordEncryptUtil
						.decryptPassword("enc:routine.encryption.key.v1:Vw+iHZ79k2Ky12im6goOIV5UBDUYz2gzl4LPa/ZIdQ==");
				...
				if (dbschema_tDBOutput_1 == null || dbschema_tDBOutput_1.trim().length() == 0) {
					tableName_tDBOutput_1 = "Marketing_spend";
				} else {
					tableName_tDBOutput_1 = dbschema_tDBOutput_1 + "].[" + "Marketing_spend";
				}
				...
				String insert_tDBOutput_1 = "INSERT INTO [" + tableName_tDBOutput_1
						+ "] ([id],[month],[marketing_spend],[new_beneficiaries]) VALUES (?,?,?,?)";
				...
				final String decryptedPassword_tFileInputExcel_1 = routines.system.PasswordEncryptUtil
						.decryptPassword("enc:routine.encryption.key.v1:XfpOrRbqM+ksVn99NcsL8d1t7WtVb8NVnE30uw==");
				...
				Object source_tFileInputExcel_1 = "C:/Users/user/Downloads/fichier excel modifié/marketing_realistic_2022_2025_with_id.xlsx";
						...
						for (int i_tFileInputExcel_1 = begin_line_tFileInputExcel_1; i_tFileInputExcel_1 < end_line_tFileInputExcel_1; i_tFileInputExcel_1++) {
							...
							try {
								...
								if (temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].length() > 0) {
									...
									row1.month = temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1];
								} else {
									row1.month = null;
									emptyColumnCount_tFileInputExcel_1++;
								}
								...
								if (temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].length() > 0) {
									...
									row1.marketing_spend = ParserUtils.parseTo_Integer(ParserUtils.parseTo_Number(
											temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1], null,
											'.' == decimalChar_tFileInputExcel_1 ? null
													: decimalChar_tFileInputExcel_1));
								} else {
									row1.marketing_spend = null;
									emptyColumnCount_tFileInputExcel_1++;
								}
								...
								if (temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].length() > 0) {
									...
									row1.new_beneficiaries = ParserUtils.parseTo_Integer(ParserUtils.parseTo_Number(
											temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1], null,
											'.' == decimalChar_tFileInputExcel_1 ? null
													: decimalChar_tFileInputExcel_1));
								} else {
									row1.new_beneficiaries = null;
									emptyColumnCount_tFileInputExcel_1++;
								}
...
// Start of branch "row1"
							if (row1 != null) {
								...
								if (row1.month == null) {
									...
									pstmt_tDBOutput_1.setString(2, row1.month);
								}

								if (row1.marketing_spend == null) {
									pstmt_tDBOutput_1.setNull(3, java.sql.Types.INTEGER);
								} else {
									pstmt_tDBOutput_1.setInt(3, row1.marketing_spend);
								}

								if (row1.new_beneficiaries == null) {
									pstmt_tDBOutput_1.setNull(4, java.sql.Types.INTEGER);
								} else {
									pstmt_tDBOutput_1.setInt(4, row1.new_beneficiaries);
								}
	...
	public static void main(String[] args) {
		final Marketing_Spend Marketing_SpendClass = new Marketing_Spend();

		int exitCode = Marketing_SpendClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = Marketing_Spend.class.getClassLoader()
					.getResourceAsStream("test/marketing_spend_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = Marketing_Spend.class.getClassLoader()
		...
		if (false) {
			System.out.println(
					(endUsedMemory - startUsedMemory) + " bytes memory increase when running : Marketing_Spend");
		}
...
/************************************************************************************************
 * 59969 characters generated by Talend Open Studio for Data Integration on the
 * 25 mars 2026, 23:10:20 WAT
 ************************************************************************************************/