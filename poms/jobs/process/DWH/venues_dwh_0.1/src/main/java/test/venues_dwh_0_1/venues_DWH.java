 ...
 * @status
 */
public class venues_DWH implements TalendJob {
	...
	private final String jobName = "venues_DWH";
				...
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(venues_DWH.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class outStruct implements routines.system.IPersistableRow<outStruct> {
		final static byte[] commonByteArrayLock_TEST_venues_DWH = new byte[0];
		static byte[] commonByteArray_TEST_venues_DWH = new byte[0];
		...
		public Integer capacity_min;

		public Integer getCapacity_min() {
			return this.capacity_min;
		}

		public Integer capacity_max;

		public Integer getCapacity_max() {
			return this.capacity_max;
		}

		public String raw_capacity;

		public String getRaw_capacity() {
		...
		public int hashCode() {
			if (this.hashCodeDirty) {
				...
				result = prime * result + ((this.id_venue == null) ? 0 : this.id_venue.hashCode());
		...
		public boolean equals(Object obj) {
			...
			if (this.id_venue == null) {
				if (other.id_venue != null)
					return false;

			} else if (!this.id_venue.equals(other.id_venue))
		...
		public void copyDataTo(outStruct other) {
			...
			other.venue_name = this.venue_name;
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_TEST_venues_DWH.length) {
					if (length < 1024 && commonByteArray_TEST_venues_DWH.length == 0) {
						commonByteArray_TEST_venues_DWH = new byte[1024];
					} else {
						commonByteArray_TEST_venues_DWH = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_TEST_venues_DWH, 0, length);
				strReturn = new String(commonByteArray_TEST_venues_DWH, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_TEST_venues_DWH, 0, length);
				strReturn = new String(commonByteArray_TEST_venues_DWH, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_TEST_venues_DWH) {
				...
				try {
					...
					this.capacity_min = readInteger(dis);
		...
		public void writeData(ObjectOutputStream dos) {
			try {
				...
				writeInteger(this.capacity_min, dos);

				// Integer

				writeInteger(this.capacity_max, dos);
		...
		public int compareTo(outStruct other) {
			...
			returnValue = checkNullsAndCompare(this.id_venue, other.id_venue);
			if (returnValue != 0) {
				return returnValue;
			}
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				if (dbschema_tDBOutput_1 == null || dbschema_tDBOutput_1.trim().length() == 0) {
					...
					tableName_tDBOutput_1 = dbschema_tDBOutput_1 + "].[" + "Dim_Venue";
				}
				...
				try {
					...
					while (rs_tDBInput_1.next()) {
						...
						if (colQtyInRs_tDBInput_1 < 1) {
							...
							if (tmpContent_tDBInput_1 != null) {
								if (talendToDBList_tDBInput_1.contains(
									...
									row1.venue_name = FormatterUtils.formatUnwithE(tmpContent_tDBInput_1);
								} else {
									row1.venue_name = tmpContent_tDBInput_1;
								}
							} else {
								row1.venue_name = null;
							}
						...
						if (colQtyInRs_tDBInput_1 < 4) {
							...
							row1.capacity_min = rs_tDBInput_1.getInt(4);
						...
						if (colQtyInRs_tDBInput_1 < 5) {
							...
							row1.capacity_max = rs_tDBInput_1.getInt(5);
							if (rs_tDBInput_1.wasNull()) {
								row1.capacity_max = null;
							}
						}
						if (colQtyInRs_tDBInput_1 < 6) {
							...
							if (tmpContent_tDBInput_1 != null) {
								if (talendToDBList_tDBInput_1.contains(
									...
									row1.raw_capacity = FormatterUtils.formatUnwithE(tmpContent_tDBInput_1);
								} else {
									row1.raw_capacity = tmpContent_tDBInput_1;
								}
							} else {
								row1.raw_capacity = null;
							}
...
// # Output table : 'out'
							out_tmp.id_venue = Numeric.sequence("seq1", 1, 1);
							out_tmp.venue_name = row1.venue_name;
							out_tmp.city = row1.city;
							out_tmp.venue_type = row1.venue_type;
...
// ###############################      
	...
	public static void main(String[] args) {
		final venues_DWH venues_DWHClass = new venues_DWH();

		int exitCode = venues_DWHClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = venues_DWH.class.getClassLoader()
					.getResourceAsStream("test/venues_dwh_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = venues_DWH.class.getClassLoader()
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : venues_DWH");
		}
