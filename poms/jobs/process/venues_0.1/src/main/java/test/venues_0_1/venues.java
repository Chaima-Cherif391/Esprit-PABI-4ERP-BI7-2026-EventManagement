 ...
 * @status
 */
public class venues implements TalendJob {
	...
	private final String jobName = "venues";
				...
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(venues.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class row1Struct implements routines.system.IPersistableRow<row1Struct> {
		final static byte[] commonByteArrayLock_TEST_venues = new byte[0];
		static byte[] commonByteArray_TEST_venues = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_TEST_venues.length) {
					if (length < 1024 && commonByteArray_TEST_venues.length == 0) {
						commonByteArray_TEST_venues = new byte[1024];
					} else {
						commonByteArray_TEST_venues = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_TEST_venues, 0, length);
				strReturn = new String(commonByteArray_TEST_venues, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_TEST_venues, 0, length);
				strReturn = new String(commonByteArray_TEST_venues, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_TEST_venues) {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				if (dbschema_tDBOutput_1 == null || dbschema_tDBOutput_1.trim().length() == 0) {
					tableName_tDBOutput_1 = "venues";
				} else {
					tableName_tDBOutput_1 = dbschema_tDBOutput_1 + "].[" + "venues";
				}
				...
				String insert_tDBOutput_1 = "INSERT INTO [" + tableName_tDBOutput_1
						+ "] ([venue_name],[city],[venue_type],[capacity_min],[capacity_max],[raw_capacity]) VALUES (?,?,?,?,?,?)";
...
// Start of branch "row1"
							if (row1 != null) {
								...
								if (row1.venue_name == null) {
									pstmt_tDBOutput_1.setNull(1, java.sql.Types.VARCHAR);
								} else {
									pstmt_tDBOutput_1.setString(1, row1.venue_name);
								}

								if (row1.city == null) {
									pstmt_tDBOutput_1.setNull(2, java.sql.Types.VARCHAR);
								} else {
									pstmt_tDBOutput_1.setString(2, row1.city);
								}

								if (row1.venue_type == null) {
									pstmt_tDBOutput_1.setNull(3, java.sql.Types.VARCHAR);
								} else {
									pstmt_tDBOutput_1.setString(3, row1.venue_type);
								}

								if (row1.capacity_min == null) {
									pstmt_tDBOutput_1.setNull(4, java.sql.Types.INTEGER);
								} else {
									pstmt_tDBOutput_1.setInt(4, row1.capacity_min);
								}

								if (row1.capacity_max == null) {
									pstmt_tDBOutput_1.setNull(5, java.sql.Types.INTEGER);
								} else {
									pstmt_tDBOutput_1.setInt(5, row1.capacity_max);
								}

								if (row1.raw_capacity == null) {
									pstmt_tDBOutput_1.setNull(6, java.sql.Types.VARCHAR);
								} else {
									pstmt_tDBOutput_1.setString(6, row1.raw_capacity);
								}
	...
	public static void main(String[] args) {
		final venues venuesClass = new venues();

		int exitCode = venuesClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = venues.class.getClassLoader()
					.getResourceAsStream("test/venues_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = venues.class.getClassLoader()
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : venues");
		}
